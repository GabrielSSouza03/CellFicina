...
function FlowSteps({ current }: { current: number }) {
  return <div className="flow-steps"><span className={`flow-step${current === 0 ? ' active' : ' done'}`}><i>1</i> Cliente</span><span className="flow-sep" /><span className={`flow-step${current === 1 ? ' active' : ''}`}><i>2</i> Ordem de serviço</span></div>
}

function NewCustomerForOrder({ onNavigate }: { onNavigate: (label: string) => void }) {
  const [name, setName] = useState('')
  return <>
    <div className="page-heading"><div><p className="eyebrow">Ordens de serviço / Nova</p><h1>Pré-cadastro do cliente</h1><p className="subheading">Cadastre o cliente para abrir a ordem de serviço.</p></div><button className="btn secondary" onClick={() => onNavigate('Ordens de Serviço')}><X size={16}/> Cancelar</button></div>
    <FlowSteps current={0} />
    <form className="form-layout single" onSubmit={(event) => { event.preventDefault(); onNavigate(`Nova Ordem de Serviço:${encodeURIComponent(name.trim() || 'Novo cliente')}`) }}>
      <div className="form-main"><div className="panel form-panel"><div className="section-title"><span className="step">01</span><div><h2>Dados do cliente</h2><p>Preencha o pré-cadastro para seguir para a OS.</p></div></div>
        <div className="form-grid">
          <label className="span-2">Nome<input value={name} onChange={(e) => setName(e.target.value)} required /></label>
          <label>CPF/CNPJ<input /></label>
          <label>Telefone<input /></label>
          <label>E-mail<input /></label>
          <label>Cidade<input /></label>
          <label className="span-2">Observações<textarea /></label>
        </div>
        <button className="btn primary" style={{ marginTop: 18 }}>Continuar para a OS</button>
      </div></div>
    </form>
  </>
}

function NewOrder({ onNavigate, clientName }: { onNavigate: (label: string) => void; clientName?: string }) {
  const name = clientName || 'Novo cliente'
  const avatar = name.split(' ').map(n => n[0]).join('').slice(0, 2)
  return <><div className="page-heading"><div><p className="eyebrow">Ordens de serviço / Nova</p><h1>Cadastro da OS</h1><p className="subheading">Cliente cadastrado. Finalize os dados da manutenção.</p></div><button className="btn secondary" onClick={() => onNavigate('Nova Ordem de Serviço')}><X size={16}/> Voltar</button></div><FlowSteps current={1} /><div className="form-layout"><div className="form-main"><div className="panel form-panel"><div className="section-title"><span className="step">01</span><div><h2>Cliente e veículo</h2><p>Quem trouxe o veículo para manutenção?</p></div></div><div className="form-grid"><label>Cliente<input value={name} readOnly /></label><label>Veículo<select><option>Honda Civic 2020 — RTA-4J82</option><option>Adicionar novo veículo</option></select></label><label>Quilometragem<input defaultValue="68.420 km" /></label><label>Responsável<select><option>Fernando Oliveira</option><option>Beatriz Santos</option></select></label></div></div><div className="panel form-panel"><div className="section-title"><span className="step">02</span><div><h2>Detalhes do atendimento</h2><p>Informações importantes sobre o serviço.</p></div></div><div className="form-grid"><label>Data de entrada<input type="date" defaultValue="2024-06-18" /></label><label>Previsão de entrega<input type="date" defaultValue="2024-06-20" /></label><label className="span-2">Diagnóstico inicial<textarea placeholder="Descreva os sintomas relatados e o diagnóstico inicial..." /></label><label className="span-2">Observações<textarea placeholder="Observações internas para a equipe..." /></label></div></div><div className="panel form-panel"><div className="section-title"><span className="step">03</span><div><h2>Serviços e peças</h2><p>Adicione os itens que serão realizados.</p></div><button className="btn secondary small"><Plus size={15}/> Adicionar item</button></div><div className="line-item"><div><strong>Revisão periódica completa</strong><small>SRV-001 · Serviço</small></div><span>1 × R$ 890,00</span><button className="row-menu"><X size={16}/></button></div><div className="line-item"><div><strong>Óleo sintético 5W30</strong><small>PRD-048 · Produto</small></div><span>4 × R$ 62,50</span><button className="row-menu"><X size={15}/></button></div></div></div><aside className="order-summary panel"><div className="summary-title"><h2>Resumo da ordem</h2><span className="status status-amber">Orçamento</span></div><div className="summary-client"><span className="avatar large">{avatar}</span><div><strong>{name}</strong><small>Honda Civic 2020 · RTA-4J82</small></div></div><div className="summary-lines"><div><span>Serviços <small>1 item</small></span><b>R$ 890,00</b></div><div><span>Peças <small>1 item</small></span><b>R$ 250,00</b></div><div><span>Desconto</span><b>R$ 0,00</b></div></div><div className="summary-total"><span>Total da ordem</span><strong>R$ 1.140,00</strong></div><button className="btn primary wide" onClick={() => onNavigate('Ordens de Serviço')}>Salvar ordem <ArrowUpRight size={16}/></button><button className="btn secondary wide" onClick={() => onNavigate('Ordens de Serviço')}>Salvar e enviar orçamento</button><p className="secure-note"><CheckCircle2 size={14}/> Você poderá editar esta ordem depois.</p></aside></div></>
}
