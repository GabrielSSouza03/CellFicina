...
function Shell() {
  ...
  const content = useMemo(() => {
    ...
    if (active === 'Nova Ordem de Serviço') return <CustomerFormPage onNavigate={navigate} continueTo="os" />
    if (active.startsWith('Nova Ordem de Serviço:')) return <WorkOrderFormPage customerId={active.slice('Nova Ordem de Serviço:'.length)} onNavigate={navigate} />
  ...
  const breadcrumb = active.startsWith('Nova Ordem de Serviço') ? 'Ordens de serviço' : active.includes(':') ? active.split(':')[0] : active
